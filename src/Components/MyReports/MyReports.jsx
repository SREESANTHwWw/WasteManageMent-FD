...
const {user} = useAuth()
  ...
  // optional: reward points from backend user
  ...
  const fetchReports = async (page = 1) => {
    try {
      ...
      if (res?.data?.success) {
        ...
        // optional (if you return it later)
        if (typeof res.data.rewardPoint !== "undefined") {
          setRewardPoint(res.data.rewardPoint);
        }
      } else {
        setReports([]);
        setTotalItems(0);
  ...
  // If your Pagination component needs totalItems for page count, use totalItems from backend
  // If it needs totalPages, pass totalPages additionally (if supported).
...
>
  ...
  <div className="relative">
    ...
    {/* Small floating sparkles around the leaf */}
    <motion.span 
  ...
  <div className="flex flex-col">
    ...
    <div className="flex items-center gap-2">
      ...
      {/* Grid */}
      {reports.length === 0 ? (
        <p className="text-center py-20">No reports found.</p>
      ) : (
        <>
          ...
          <Pagination
            totalItems={totalItems} // ✅ backend total count
